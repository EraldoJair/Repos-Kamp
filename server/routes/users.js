...
router.get('/all', 
  ...
  async (req, res) => {
    try {
      const users = await User.find({}, 'profile.firstName profile.lastName');
      res.json({ users });
    } catch (error) {
...
router.get('/', 
  authenticate, 
  authorize('jefe_mantenimiento', 'superintendent_operaciones', 'gerente_financiero', 'gerente_general', 'admin'), 
  async (req, res) => {
    try {
...
router.post('/', 
  ...
  async (req, res) => {
    try {
      ...
      } else if (error.code === 11000) { // Duplicate key error (e.g., email or employeeId already exists)
        const field = Object.keys(error.keyValue)[0];
        return res.status(400).json({ message: `El ${field} ya existe.`, field });
      }

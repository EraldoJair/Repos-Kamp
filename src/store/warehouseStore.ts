...
export interface Warehouse {
  ...
  type: 'principal' | 'satelite' | 'movil' | 'temporal';
  responsible: {
    userId: string;
    name: string;
  };
...
interface WarehouseStore {
  ...
  createWarehouse: (warehouseData: Partial<Warehouse>) => Promise<void>;
  ...
  createWarehouse: async (warehouseData) => {
    ...
    try {
      ...
      if (response.ok) {
        const data = await response.json();
        set(state => ({
          warehouses: [data.warehouse, ...state.warehouses],
          loading: false
        }));
      } else {
        const errorData = await response.json();
        set({ error: errorData.message || 'Failed to create warehouse', loading: false });
        throw new Error(errorData.message);
      }
    } catch (error) {
      set({ error: 'Network error creating warehouse', loading: false });
      throw error;
    }
  },
